...
_OT = namedtuple("opt_task", "tup value unsatH terminal") # tup is the initial seq
class opt_task(_OT, Node):
    def find_children(board,action):
        ...
        for index in action:
            ...
            if flip <= 0:
                ...
                if board.unsatH == 0:
                    ...
                    p = np.zeros(seq_len)
                    for i in board.unsatH.split('_'):
                        p[int(i)-1] += 1
                    p = p / np.sum(p)
                    index_2 = np.random.choice(seq_len, np.count_nonzero(p), p=p)
                    for i in index_2:
        ...
        is_terminal=False
        return {opt_task(tuple(t), v, u, is_terminal) for t, v, u in zip(all_tup, all_value, all_unsatH)}
...
def single_run(initial_X,top_n):
  ...
  print(initial_X, values, unsatH)
  exp_weight = 0.8 * values
  if exp_weight < 1:
      exp_weight = 1
  board_uct = opt_task(tup=tuple(initial_X), value=values, unsatH=unsatH, terminal=False)
  rollout_round = 15
