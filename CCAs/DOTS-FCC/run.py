...
##[run times, top start points, random start points, top score samples, top rank samples, top visit samples, random samples]
list1=[5,8,2,5,3,1,1]


#random component database
step_size = 0.5
Fe_low = 2
...
# plt.figure()
# plt.scatter(data[:,n_dim+2],data[:,n_dim+4],label='all data')
# plt.xlabel('ahc')
# plt.ylabel('aha')
# plt.title("aha vs ahc")
# plt.legend()
...
def model_training(X,y,name,i):
      ...
      model.fit(X_train, y_train, validation_data=(X_test, y_test),  batch_size=50, epochs=5000, callbacks=[es,mc])
...
def combine(x_proposed):
    x1 = emsemble_pred1(x_proposed)
    x2 = emsemble_pred2(x_proposed)
    xx1 = (x1/10000)/(0.01/x2)
    combined = np.concatenate((x_proposed,
                               np.zeros((len(x_proposed), 1)),
                               x1.reshape(len(x_proposed), 1),
                               x2.reshape(len(x_proposed), 1)/10000,
                               xx1.reshape(len(x_proposed), 1),
                               ), axis=1)
    return combined
...
def FCN_ratiocheck(x_input):
    ...
    # FCN_ratio = Fe_num/(Co_num + Ni_num)
    # if FCN_ratio > 2.5:
    #     ratio = 2.5
    # elif FCN_ratio < 1.5:
    #     ratio = 1.5    
...
def pareto_evaluation(all_input, sample_input,num,plot_save = False): #Euclidean distance + pred score #pareto front
    ...
    # print(pareto_front)
    # print(sample_score[pareto_front])
    
    ind = np.random.choice(pareto_front,num,replace=False)
    # print(ind)
    # print(oracle(sample_input[ind]))
    # print(sample_score[ind])
...
def single_run(X,y,initial_X,greedy_UCT,UCT_low):#[ 3*3*3 matrices, yield, model1, model2, top 3*3*3 matrix,greedy_UCT,UCT_low]
    ...
    # ind2=TSNEPCA(all_input, boards_rand, oracle(boards_rand).reshape(-1),list1[4])
    # ind2=dist_evaluation(all_input, boards_rand,list1[4])
...
# top_all4=TSNEPCA2(top_all3, np.array(sample_score).reshape(-1),100)
# ind=TSNEPCA(all_input, top_all3, np.array(sample_score).reshape(-1),top_sample,plot_save=True) # compared to initial data
# top_select = top_all3[ind]
