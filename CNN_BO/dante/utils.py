...
class Tracker:
    ...
    def dump_trace(self) -> None:
        ...
        np.save(
            f"{self.folder_name}/{self.method}-result{self.dims}d-{self.i}.npy", np.array(self._results), allow_pickle=True
        )
        np.save(
            f"{self.folder_name}/{self.method}-input{self.dims}d-{self.i}.npy", np.array(self._x_values), allow_pickle=True
        )
    ...
    def track(
        ...
        if save or self._counter % 10 == 0 or round(self._current_best, 5) == 0:
            self.dump_trace()
