...
if args.method == 'DOTS':
    ...
    tree_ubt = MCTS_ubt(exploration_weight=exp_weight, f=f, name=args.func)
    for i in range(args.iterations):
        ...
        if i % int(1e9/(args.dims**2)) == 0:
            tree_ubt = MCTS_ubt(exploration_weight=ratio * round(1000/(fy+0.1),10), f=f, name=args.func)
    ...
    optimizer = MCTS_Greedy(f = f, dims = args.dims)
    for i in range(args.iterations):
    ...
    optimizer = MCTS_eGreedy(f = f, dims = args.dims)
    for i in range(args.iterations):
        node_new, temp_Y = optimizer.choose(init_X)
